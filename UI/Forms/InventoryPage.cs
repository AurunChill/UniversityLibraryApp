...
using Loc = LibraryApp.Data.Models.Location;
...
public sealed class InventoryPage : TablePageBase
{
    ...
    private string _locSort = nameof(Loc.LocationId);
    ...
    private async Task LoadLocationsAsync(string? sortColumn = null)
    {
        ...
        {
            nameof(Loc.LocationName) => _locOrder == SortOrder.Ascending ? list.OrderBy(l => l.LocationName).ToList() : list.OrderByDescending(l => l.LocationName).ToList(),
            nameof(Loc.Amount) => _locOrder == SortOrder.Ascending ? list.OrderBy(l => l.Amount).ToList() : list.OrderByDescending(l => l.Amount).ToList(),
...
internal sealed class LocationDialog : Form
{
    ...
    public LocationDialog(LocationService service, Location? existing = null)
    {
        ...
        ok.Click += async (_, __) =>
        {
            ...
            if (_orig is null)
            {
                await _service.AddAsync(new Loc { LocationName = tName.Text, Amount = 0 });
            }
            else
            {
...
internal sealed class TransactionAddPage : Form
{
    ...
    private async Task LoadLocationsAsync()
    {
        ...
        cbTo.DisplayMember = nameof(Loc.LocationName);
        cbFrom.DataSource = list.ToList();
        cbFrom.DisplayMember = nameof(Loc.LocationName);
    }
    ...
    {
        if (cbTo.SelectedItem is not Loc to || cbFrom.SelectedItem is not Loc from)
        {
