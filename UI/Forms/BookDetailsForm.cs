...
{
    private Book _book;
    private PictureBox _cover = null!;          // нужен для Dispose
    ...
    private void BuildUI()
    {
        ...
        Controls.AddRange([btnDel, btnEdit]);
    }
    ...
    /* загружаем картинку без блокировки файла */
    private static Image? LoadImageUnlocked(string path)
    {
        try
        {
            using var fs = new FileStream(path, FileMode.Open, FileAccess.Read);
            using var img = Image.FromStream(fs);         // Stream закрывается → файл свободен
            return (Image)img.Clone();                    // клонируем в память
        }
        catch { return null; }
    }
    ...
    private void EditBook()
    {
        ...
        // обновляем модель и перерисовываем
...
internal sealed class BookEditDialog : Form
{
    ...
    void BuildUI()
    {
        Text = "Редактирование"; Size = new(520, 530); BackColor = Color.FromArgb(40, 40, 46);
        ForeColor = Color.Gainsboro; Font = new("Segoe UI", 10); StartPosition = FormStartPosition.CenterParent;
        int y = 20;
        Controls.AddRange([
        ...
        // заполняем
        tTitle.Text = _orig.Title; tAuthor.Text = _orig.Author; tIsbn.Text = _orig.ISBN;
        nYear.Value = _orig.PublishYear; nPages.Value = _orig.Pages;
        tGenre.Text = _orig.Genre; tPublisher.Text = _orig.Publisher;
        tLang.Text = _orig.Language; tLoc.Text = _orig.Location; tDesc.Text = _orig.Description;
        ...
        ok.Click += async (_, __) =>
        {
            ...
            {
                MessageBox.Show("Название не может быть пустым"); DialogResult = DialogResult.None; return;
            }
            // копируем изменения
            var book = _orig;
            book.Title = tTitle.Text; book.Author = tAuthor.Text; book.ISBN = tIsbn.Text;
            book.PublishYear = (int)nYear.Value; book.Pages = (int)nPages.Value;
            book.Genre = tGenre.Text; book.Publisher = tPublisher.Text;
            book.Language = tLang.Text; book.Location = tLoc.Text; book.Description = tDesc.Text;
    ...
    private static void ResizeCrop260x320(string src, string dest)
    {
        ...
        int nw = 260, nh = (int)(img.Height * (nw / (double)img.Width));
        ...
        {
            g.Clear(Color.White);
            if (nh >= 320) { int y = (nh - 320) / 2; g.DrawImage(bmp, 0, 0, new Rectangle(0, y, 260, 320), GraphicsUnit.Pixel); }
            else { int yPad = (320 - nh) / 2; g.DrawImage(bmp, 0, yPad); }
        }
