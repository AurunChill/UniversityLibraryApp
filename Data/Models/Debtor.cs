...
public static class Debtors
{
    ...
    public static async Task<IReadOnlyList<Debtor>> GetAllAsync(int? limit=null,int off=0)
    {
        ...
        var q=db.Debtors.Include(d=>d.Book).Include(d=>d.ReaderTicket)
                        .AsNoTracking().Skip(off);
    ...
    { await using var db=await Factory.CreateDbContextAsync();
      return await db.Debtors.Include(d=>d.Book).Include(d=>d.ReaderTicket)
                             .AsNoTracking().FirstOrDefaultAsync(x=>x.DebtorId==id); }

    public static async Task<bool> DeleteAsync(long id)
    { await using var db=await Factory.CreateDbContextAsync();
      var e=await db.Debtors.FindAsync(id); if(e is null)return false;
      db.Debtors.Remove(e); return await db.SaveChangesAsync()>0; }

    /* создание / обновление с логикой */
    public static async Task<Debtor> AddAsync(Debtor d)
    {
        ...
        await LogSupplyIfReturnedAsync(db,d);
        return d;
    }
    public static async Task<bool> UpdateAsync(Debtor d)
    {
        ...
        if(ok) await LogSupplyIfReturnedAsync(db,d);
        return ok;
    }

    private static async Task LogSupplyIfReturnedAsync(LibraryContext db, Debtor d)
    {
        if(d.Status==DebtorStatus.В_Срок.Text() ||
    ...
    public static async Task<IReadOnlyList<Debtor>> GetOpenDebtsAsync()
    {
        await using var db=await Factory.CreateDbContextAsync();
        return await db.Debtors.Include(d=>d.Book).Include(d=>d.ReaderTicket)
            .Where(d=> d.Status==DebtorStatus.Просрочено.Text() ||
                       d.Status==DebtorStatus.Возвращено_Без_Оплаты.Text() ||
    ...
    {
        await using var db=await Factory.CreateDbContextAsync();
        var list= db.Debtors.Where(d=> d.Status==DebtorStatus.В_Срок.Text() ||
                                       d.Status==DebtorStatus.Просрочено_Возвращено_С_Оплатой.Text());
    ...
    {
        await using var db=await Factory.CreateDbContextAsync();
        return await db.Debtors.Include(d=>d.Book)
            .Where(d=>d.ReaderTicketId==ticketId &&
    ...
    private static int CalcDays(Debtor d)
    {
        ...
        var span    = refDt.ToDateTime(TimeOnly.MinValue)
                    - d.DebtDate.ToDateTime(TimeOnly.MinValue);
        return span.Days;
    }
}
